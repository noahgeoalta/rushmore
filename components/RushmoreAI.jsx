...
// Targets the bloom div directly — sets background radial-gradient inline every RAF frame.
// Bloom is z-index above video with mix-blend-mode:screen — glow is inside the frame.
  ...
  useEffect(() => {
    const loop = () => {
      ...
      if (!analyserNode) {
        ...
        el.style.background =
          `radial-gradient(ellipse 75% 75% at 50% 50%, rgba(30,200,30,${a}) 0%, rgba(20,160,20,${b}) 40%, transparent 72%)`;
        return;
      }
      ...
      const a1 = Math.min(0.85, 0.15 + rms * 0.70).toFixed(2);
      const a2 = Math.min(0.45, 0.06 + rms * 0.39).toFixed(2);
      const a3 = Math.min(0.15, 0.02 + rms * 0.13).toFixed(2);
      el.style.background =
        `radial-gradient(ellipse 80% 80% at 50% 50%, rgba(${r},${g},${b2},${a1}) 0%, rgba(${r},${g},${b2},${a2}) 45%, rgba(${r},${g},${b2},${a3}) 65%, transparent 80%)`;
    };
    rafRef.current = requestAnimationFrame(loop);
...
export default function RushmoreAI() {
  ...
  return (
    <div className="ai-shell">
      ...
      {/* Full-width black strip — video frame centred inside at max 900px */}
      <div className="ai-video-strip">
        <div className="ai-video-sticky">
          ...
          />
          {/* Bloom overlay — ON TOP of video via z-index, mix-blend-mode:screen */}
